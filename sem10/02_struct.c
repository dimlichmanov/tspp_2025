...
int main(int argc, char *argv[])
{
    ...
    /* Просто посмотрим, что за тип */
    if (my_rank == 0) {
        int pers_size;
        MPI_Aint pers_extent;
        MPI_Aint lb;
        struct person_t some_strange_person;

        MPI_Type_size(person_type, &pers_size);
        MPI_Type_get_extent(person_type, &lb, &pers_extent);
        
        printf("Pers size: %d, extent: %ld, lower bound: %ld, sizeof(): %ld \n", pers_size, pers_extent, lb, sizeof(some_strange_person));
    }
