...
int main(int argc, char** argv) {
    ...
    for (int iter = 0; iter < n_iters; iter++) {
        ...
        /* ВНИМАНИЕ! Более чем достаточно будет сделать Waitall для всех отправок/приёмов, и затем обрабатывать ВСЕ крайние элементы. 
        НЕ нужно оптимизировать дальше (в частности, можно дождаться сначала левой плоскости и обработать её, потом правой..)
        Это раздует ваш код до неприличных размеров. Самая базовая идея как это сделать +- читабельно в ОДНОМ цикле - ниже */
